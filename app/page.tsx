...
export default function Home() {
  ...
  return (
    <div className="flex flex-col" style={{ height: '100dvh', overflow: 'hidden', background: '#0B1220' }}>
      {/* Top toggle bar — dark, minimal */}
      <div className="flex items-center gap-4 px-5 py-2.5 border-b border-white/[0.06] shrink-0" style={{ background: '#0A0F1E' }}>
        <div className="flex items-center gap-2 mr-2">
          <span className="inline-flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-pill bg-cyan-300" />
            <span className="text-[10.5px] font-semibold tracking-wider2 text-white/85">LOFTY AI</span>
          </span>
          <span className="text-white/15 text-xs">/</span>
          <span className="text-[10px] text-white/35 font-medium tracking-tight hidden md:inline">GlobeHack '26 demo</span>
        </div>

        <div className="flex items-center bg-white/[0.04] border border-white/[0.06] rounded-pill p-0.5 gap-0.5">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              onClick={() => goTo(tab.id)}
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="hidden md:inline text-[10px] text-white/30 font-medium tracking-tight">
